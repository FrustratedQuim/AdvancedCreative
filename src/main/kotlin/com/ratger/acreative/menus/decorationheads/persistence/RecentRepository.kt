    ...
    fun push(playerId: UUID, entry: Entry) {
        ...
        database.connection().use { conn ->
            ...
            conn.prepareStatement(
                """
                INSERT INTO decoration_head_recent (player_uuid, position, last_used_at, stable_key, api_id, head_name, category_id, texture_value)
                VALUES (?, 0, ?, ?, ?, ?, ?, ?)
                """.trimIndent()
            ).use { ps ->
                ps.setString(1, player)
                ...
                ps.setObject(4, entry.apiId)
                ps.setString(5, entry.name)
                ps.setInt(6, entry.categoryId)
                ps.setString(7, entry.textureValue)
                ps.executeUpdate()
            }
