package com.ratger.acreative.menus.itemEdit.pages.advanced
...
class AdvancedEditDetailsPage(
    ...
    private fun buildRarityButton(session: ItemEditSession): Button {
    ...
    private fun buildAttributesButton(player: Player, session: ItemEditSession): Button {
    ...
    fun open(player: Player, session: ItemEditSession) {
        ...
        menu.setButton(40, buttonFactory.actionButton(
            ...
            lore = listOf("<!i><#FFD700>Нажмите, <#FFE68A>чтобы открыть"),
            action = { support.transition(session) { openUseCooldownPage(player, session) } }
        ))
